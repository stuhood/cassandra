...
public class SSTableWriterTest extends CleanupHelper {
    ...
    private void testRecoverAndOpenIndexed(String ksname, String cfname) throws IOException, ExecutionException, InterruptedException
    {
        ...
        cf.addColumn(new Column(Util.bytes("birthdate"), ByteBufferUtil.bytes(1L), 0));
        cf.addColumn(new Column(Util.bytes("anydate"), ByteBufferUtil.bytes(1L), 0));
        entries.put("k2", cf);        
        ...
        SSTableReader orig = SSTableUtils.prepare().ks(ksname).cf(cfname).write(entries);
        ...
        Descriptor desc = CompactionManager.instance.submitSSTableBuild(cfs, orig.descriptor, Component.INDEX_TYPES, OperationType.AES).get().left;
