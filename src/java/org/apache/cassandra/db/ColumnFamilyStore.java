...
// TODO this actually isn't a good meature of pending tasks
    ...
    public SortedSet<ByteBuffer> getIndexedColumns(IndexType type)
    {
        ...
        for (Map.Entry<ByteBuffer,SecondaryIndex> secindex : indexedColumns.entrySet())
            if (secindex.getValue().cdef.getIndexType() == type)
                columns.add(secindex.getKey());
        return columns;
    }

    public SecondaryIndex getSecondaryIndex(ByteBuffer column)
    {
    ...
    /**
     * @return A list of the names of the built column indexes for current store
     */
    public List<String> getBuiltIndexes()
    {
        List<String> indexes = new ArrayList<String>();
        for (SecondaryIndex sidx : indexedColumns.values())
            if (sidx.isBuilt())
                indexes.add(sidx.getName()); 
        return indexes;
    }
