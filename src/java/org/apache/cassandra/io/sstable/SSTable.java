...
public abstract class SSTable
{
    ...
    static long estimateRowsFromData(CFMetaData cfm, IPartitioner partitioner, Descriptor desc, RandomAccessReader dfile) throws IOException
    {
        ...
        int keys = 0;
        long dataPosition = 0;
        Cursor cursor = new Cursor(desc, cfm.getTypes());
        // SSTableScanner is built for this type of iteration, but it currently requires an open SSTableReader
        while (dataPosition < BYTES_CAP && keys < SAMPLES_CAP)
        {
            // open the row and the close it to consume the remainder, which is part of the SSTII.close() contract
            SSTableIdentityIterator iter = SSTableIdentityIterator.create(cfm,
                                                                          partitioner,
                                                                          desc,
                                                                          dfile,
                                                                          cursor,
                                                                          false);
            iter.close();
            dataPosition = dfile.getFilePointer();
            keys++;
        }
        dfile.seek(0);
